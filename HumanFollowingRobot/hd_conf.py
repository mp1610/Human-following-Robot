...
class HumanDetectionNode(Node):
    ...
    def draw_boxes(self, image, persons):
        ...
        person_detected = False  # Flag to check if at least one person is detected

        for person in persons:
            label, score, x1, y1, x2, y2 = person
            if score > 0.1:
                person_detected = True  # Set the flag to True if any person with score > 0.1 is detected
        ...
        if not person_detected:
            print("No person detected")  # Print the message if no person with score > 0.1 is detected
    ...
    def detect_persons(self, image):
        ...
        pred_instances = result.pred_instances  # Assuming this is how you access the pred_instances field
